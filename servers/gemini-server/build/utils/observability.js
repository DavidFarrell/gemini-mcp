...
export class ObservabilityCollector {
     ...
     */
    generateFingerprint(toolName, model, args) {
        const fingerprint = {
            model,
            tool_name: toolName,
        ...
        if (toolName === 'gemini_generate') {
            ...
            let staticRatio = 0;
            if (fingerprint.has_system_instruction)
                staticRatio += 0.2;
            if (fingerprint.has_tools)
                staticRatio += 0.3;
            if ((fingerprint.message_count || 0) > 3)
                staticRatio += 0.3; // Older messages become static
    ...
    getMetrics(hours = 24) {
        ...
        const totals = recentEvents.reduce((acc, event) => {
            acc.events++;
            if (event.usage)
                acc.tokens += event.usage.total_tokens;
            if (event.cost)
                acc.cost += event.cost.total_cost;
            if (event.latency_ms) {
        ...
        for (const event of recentEvents) {
            ...
            modelStat.count++;
            if (event.cost)
                modelStat.cost += event.cost.total_cost;
            // Tool stats
            ...
            toolStat.count++;
            if (event.cost)
                toolStat.cost += event.cost.total_cost;
        }
        return {
