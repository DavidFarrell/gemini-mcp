...
const mimeTypeSchema = z
  ...
  .max(255)
  .describe(
    "IANA-compliant MIME type, e.g., 'image/png', 'application/pdf', or 'application/octet-stream'."
  );

// Max decoded upload size: 50 MiB
const MAX_UPLOAD_BYTES = 50 * 1024 * 1024;

function approximateBase64DecodedBytes(b64: string): number {
...
const RetentionPolicySchema = z
    ...
    z
      .object({
        ...
        ttlSeconds: z
          ...
          .optional()
          .describe(
            "Time-to-live in seconds. Min 60s, max 30 days. Provide either ttlSeconds or expiresAt."
          ),
        expiresAt: z
          .string()
          .datetime()
          .optional()
          .describe(
            "RFC3339 timestamp for expiration. Provide either ttlSeconds or expiresAt."
          ),
      })
      .superRefine((v, ctx) => {
  ...
  .describe(
    "Retention policy for the uploaded file. Defaults to persistent if unspecified."
  );
...
const SourceBase64Schema = z
  .object({
    base64: z
      ...
      .min(4)
      .describe(
        "Base64-encoded file content. Decoded payload must not exceed 50 MiB."
      ),
  })
  .superRefine((v, ctx) => {
...
export const GeminiFileSchema = z
  .object({
    ...
    uri: z
      ...
      .max(2048)
      .describe(
        "Canonical URI for the file resource (may be used to reference it in prompts)."
      ),
    ...
    expiresAt: z
      ...
      .optional()
      .describe(
        "RFC3339 timestamp when the file will expire, if applicable (temporary files only)."
      ),
    displayName: z
      .string()
      .min(1)
...
export const GeminiUploadFileSchema = z
  .object({
    ...
    mimeType: mimeTypeSchema.describe(
      "MIME type of the file being uploaded. Required for base64; for URL, used as a hint/fallback."
    ),
    displayName: z
      .string()
      .min(1)
    ...
    dedupe: z
      ...
      .default(true)
      .describe(
        "If true (default), reuse an existing stored file with identical content when possible."
      ),
    retention: RetentionPolicySchema.default({ type: "persistent" }).describe(
      "Retention policy for the uploaded file. Defaults to persistent."
    ),
  })
  .strict()
...
export const GeminiListFilesSchema = z
  .object({
    ...
    pageToken: z
      ...
      .optional()
      .describe(
        "Opaque token to retrieve the next page of results from a previous list request."
      ),
    filter: z
      .object({
        mimeTypes: z
        ...
        displayNameContains: z
          ...
          .optional()
          .describe(
            "Case-insensitive substring to match against the displayName."
          ),
        createdAfter: z
          .string()
          .datetime()
        ...
        createdBefore: z
          ...
          .optional()
          .describe(
            "Return files created strictly before this RFC3339 timestamp."
          ),
        expired: z
          .boolean()
          .optional()
          .describe(
            "If true, return only expired files; if false, only non-expired; if omitted, return all."
          ),
      })
      .partial()
      .default({})
...
export const GeminiDeleteFileSchema = z
  .object({
    ...
    force: z
      ...
      .default(false)
      .describe(
        "If true, delete even if the file is currently referenced by other resources."
      ),
  })
  .strict()
...
export const GeminiEmbeddingsSchema = z
  .object({
    texts: z
      ...
      .max(2048)
      .describe(
        "Array of input texts to embed. Each text up to 100k characters. Max 2048 texts per request."
      ),
    model: z
      .string()
      .min(1)
      ...
      .describe(
        "Embedding model name. Defaults to 'text-embedding-004'. Use an appropriate model available in your environment."
      ),
    taskType: z
      .enum([
        "unspecified",
      ...
      .default("unspecified")
      .describe(
        "Intended downstream task to tailor embeddings. Defaults to 'unspecified'."
      ),
    truncate: z
      .enum(["NONE", "START", "END"])
      .default("NONE")
      .describe(
        "Truncation strategy if input exceeds model limits. NONE (default), START, or END."
      ),
    outputDimensionality: z
      ...
      .optional()
      .describe(
        "If set, request embeddings projected to this dimensionality. Range 8–3072 depending on model."
      ),
  })
  .strict()
...
// Additional Wave 1 type exports
