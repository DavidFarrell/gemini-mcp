...
import { GeminiGenerateSchema, GeminiMessagesSchema, GeminiUploadFileSchema, GeminiListFilesSchema, GeminiDeleteFileSchema, GeminiEmbeddingsSchema, GeminiGetDocsSchema } from './schemas.js';
import { GeminiClient, extractText, extractUsage, extractFunctionCalls, isBlocked, getBlockReason } from './providers/gemini.js';
        ...
        switch (part.type) {
            ...
            case 'video_url':
                try {
                    ...
                    const mediaType = part.type.replace('_url', '');
            ...
            case 'file_uri':
                ...
                break;
            default:
                console.error(`Unknown content part type: ${part.type}`);
        }
    }
    return parts;
}
async function mapMessagesToGeminiContents(messages, systemInstruction) {
    const contents = [];
        ...
        try {
            ...
            switch (request.params.name) {
                case "gemini_generate": {
                    ...
                    const args = GeminiGenerateSchema.parse(request.params.arguments);
                ...
                case "gemini_messages": {
                    const startTime = Date.now();
                    const args = GeminiMessagesSchema.parse(request.params.arguments);
                    const model = args.model || "gemini-2.5-flash";
                    ...
                    try {
                        ...
                        const { contents, system_instruction } = await mapMessagesToGeminiContents(args.messages, args.system);
                        // Build Gemini request
                        const geminiRequest = {
                            model,
                            system_instruction,
                ...
                case "gemini_upload_file": {
                    const args = GeminiUploadFileSchema.parse(request.params.arguments);
                ...
                case "gemini_list_files": {
                    const args = GeminiListFilesSchema.parse(request.params.arguments);
                ...
                case "gemini_delete_file": {
                    const args = GeminiDeleteFileSchema.parse(request.params.arguments);
                ...
                case "gemini_embeddings": {
                    const startTime = Date.now();
                    const args = GeminiEmbeddingsSchema.parse(request.params.arguments);
                ...
                case "gemini_getdocs": {
                    const args = GeminiGetDocsSchema.parse(request.params.arguments);
